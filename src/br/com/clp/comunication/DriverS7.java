...
//*******Método ReadArea(Identificação de Área, Número do Bloco DB, Número do endereço na DB e Buffer)********
	...
	public void getBit(List<TextField> listFieldValues, List<TextField> listFieldDB, int index) {
		try {
			client.ReadArea(S7.S7AreaDB, 
					...
					this.AMOUNTWORD, 
					this.buffer);
			boolean bit = S7.GetBitAt(this.buffer, 
					                  0, 
					                  FilterStringDB.getBitIfX(listFieldDB.get(index).getText()));
	...
	public void getDouble(Integer DB, Integer adress, int a) {
		try {
			client.ReadArea(S7.S7AreaDB, DB, adress, a, this.buffer);
			boolean data = S7.GetBitAt(this.buffer, 0, 0);
